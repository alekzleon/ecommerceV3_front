...
const DEFAULT_PUBLIC_PLANS = [
  ...
  },
  {
    ...
    name: "A la medida",
    label: "Conoce mas",
    description: "Plan personalizado para requerimientos especiales o integraciones especificas.",
    features: [
      "Integraciones especiales.",
      "Flujos a la medida.",
      "Acompanamiento para tu operacion.",
    ],
    custom: true,
  },
];
...
export function CloudiShopMarketingHome({ createStorePath = "/registro", loginPath = "/login" }) {
  ...
  const plansMarkup = useMemo(
    () => buildPublicPlansMarkup(plans, createStorePath),
...
function normalizePublicPlans(response) {
  ...
  return rawPlans.map((plan) => ({
    ...
    label: plan.label || formatPlanPrice(plan),
    price: plan.price,
...
function buildPublicPlansMarkup(plans, createStorePath) {
  ...
  return safePlans.map((plan) => {
    ...
    const badge = plan.popular
      ? `<span class="plan-badge">${escapeHtml(plan.name)}</span><span class="plan-badge alt">Popular</span>`
      : `<span class="plan-badge">${escapeHtml(plan.name)}</span>`;
    ...
    return `
          <article class="${cardClass}">
            ...
            <div class="plan-price">${escapeHtml(plan.label || formatPlanPrice(plan))}</div>
